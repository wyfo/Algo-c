...
static void class_token_clean(const void* reader) {}

static struct ReadingResult class_token_epsilon(const void* reader) {
    return (struct ReadingResult){NULL, {CLONE(reader), &class_token_vtable}};
}
static struct ReadingResult class_token_read(const void* reader, char token) {
...
static inline struct Reader class_token_reader_of(size_t size, char class_[], size_t class_size, tag_t tag, struct ReadingResult class_value, struct ReadingResult default_) {
    ...
    return (struct Reader){ptr, &class_token_vtable};
}
